...
<html>
  ...
  <?php include('school_path_survey.php');
    ...
    function find_path(sel_sch,ls){
      ...
      $("#mov").click(function(){
        //alert($("#unasso option:selected").val());
        if($("#sch option:selected").val()==-1 || $("#pth option:selected").val()==-1){
          ...
          if($("#unasso option:selected").val()==undefined){
            ...
            $("#alrmodal").find("h4").text("Do you really want to create the following association ?");
            $("#alrmodal").find("p").text("School: "+$("#sch option:selected").text()+", Path: "+$("#pth option:selected").text()+", Survey: "+$("#unasso option:selected").text());
            $("#alrmodal").modal('show');
            $(document).on("click","#sub_asso",function(){
                  $.ajax({
                    url:"association.php",
                    data:{a_survey:$("#unasso option:selected").val(),pth:$("#pth option:selected").val()},
                    type:"POST",
                    async:false
                  });
                <?php if(isset($_POST['a_survey'])){
                  association_add($_POST['pth'],$_POST['a_survey']);} ?>
      ...
      $("#remov").click(function(){
        //alert($("#asso option:selected").val());
        if($("#sch option:selected").val()==-1 || $("#pth option:selected").val()==-1){
          $("#alrmodal_1").find('h4').text("Please select at least one school and one path");
          $("#alrmodal_1").modal('show');
        }else{
          if($("#asso option:selected").val()==undefined){
            ...
            $(document).on("click","#sub_del",function(){
                $.ajax({
                    url:"association.php",
                    data:{u_survey:$("#asso option:selected").val(),pth:$("#pth option:selected").val()},
                    type:"POST",
                    async:false
                  });
                  <?php if(isset($_POST['u_survey'])){
                    association_delete($_POST['pth'], $_POST['u_survey']);} ?>
                  window.location.href="association.php";
              });
          }
        }
    ...
    <div id="tree"> </div>
      ...
      <div class="tab-content">
        <div class="tab-pane fade in active" id="manage"><br>
          <div class="row">
            <div class="col-md-5">
            <span class="info">Unassociated Surveys</span><br><br>
            <select class="form-control" size="17" id="unasso" ></select></div>
            ...
            <div class="col-md-5">
              ...
              <select class="form-control" size="10" id="asso"></select>
            </div>
          </div>
        </div>
      </div>
