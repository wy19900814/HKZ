...
function getCatagory(sur_index){
    ...
    if($("#back_to_surveys_catagory_"+sur_index).length>0){
    	$("#back_to_surveys_catagory_"+sur_index).remove();
        //alert("back_to_surveys_catagory");
    }
    if($("#catagoryUrl_"+sur_index).length>0){
    	$("#catagoryUrl_"+sur_index).remove();
    	//alert("catagoryUrl");
    }
	...
	//alert(num_b);
	var page = "<div data-role='page' id='catagoryUrl_"+sur_index+"' data-url=catagoryUrl_"+sur_index+" data-theme='c'><div data-role='header' data-theme='b'><a href='#' data-theme='b' data-icon='back' class='ui-btn-left' id='back_to_surveys_catagory_"+survey_index+"'>back</a><h1>" + "Catagory" + "</h1></div><div data-role='content'><ul data-role='listview' data-inset='false' id='survey-list'>";
...
//{"s_id":"46790148","s_name":"HKZ_ONE"}
    ...
    $.getJSON(url,{s_id:survey_id},function(data){
    	...
    	//jsonObj_ques.Questions.Blocks[ques_num].options
    	for (var i = 0; i < num_b&&jsonObj_ques.Questions.Blocks.length>0; i++) {
        	var j=i+1;
            page = page + '<li><a href="javascript:void(0)" onclick="getBlockFirstQues(\'' + i + '\')">' + 'block' +j+ '</a></li>';
            //alert("hello");
        }
    	
    	if(jsonObj_ques.Questions.Tallies.length>0){
    		page = page + '<li><a href="javascript:void(0)" onclick="getTallyQues(0)">' + 'Tally' +'</a></li>';
    	}
    	if(jsonObj_ques.Questions.Others.length>0){
    		page = page + '<li><a href="javascript:void(0)" onclick="getOtherQues(0)">' + 'Other' + '</a></li>';
    	}
    	if(jsonObj_ques.Questions.Blocks.length==0&&jsonObj_ques.Questions.Tallies.length==0&&jsonObj_ques.Questions.Others.length==0){
    		alert("There is no questions in this survey!")
    	}else{
    		page+="</ul></div>";
            ...
            $.mobile.changePage(newPage);
            submit();
            $("#back_to_surveys_catagory_"+survey_index).click(function(){
