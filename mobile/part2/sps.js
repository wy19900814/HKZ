...
function getSchools() {
    ...
    ajax.open('GET', ('http://localhost/mobile/part2/test.php')); //is completed
...
function getCatagory(survey_index) {
    ...
    var ajax = new XMLHttpRequest();
    ajax.open('GET', ('http://localhost/mobile/part2/question_test.php')); 
    ajax.send();
    //alert("hello1");
    ajax.onreadystatechange = function() {
        if (ajax.readyState == 4 && (ajax.status == 200)) { 
...
function getBlockQues(block_num){
    var page = "<div data-role='page' data-url=blockQuesUrl data-theme='c' data-add-back-btn='true' id='blockques'><div data-role='header' data-theme='b'><h1>" + "HKZ" + "</h1></div><div data-role='content'>";

    
    var questions=catagory.Questions;
    var i=0;//指的是第i个问题
    var options=catagory.Questions[0].Blocks[i].Options;
    
    page=page+'<label>'+questions[0].Blocks[i].q_heading+'</label>';
    page=page+"<form>";
    if(questions[0].Blocks[i].q_type==1){
    ...
    else if(questions[0].Blocks[i].q_type==2){
        for (var i = 0; i < options.length; i++) {
